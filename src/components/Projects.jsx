...
export default function Projects() {
  ...
  const projects = [
    ...
    {
      title: 'Revit Plugin for MEP Automation',
      company: 'Personal Project (2022)',
      description:
        'Built a Revit plugin using Revit API to automate drawing of MEP elements (ducts and pipes) from referenced DWG/DXF files.',
      fullDescription: [
        'Implemented functionality to generate MEP layouts from referenced AutoCAD DWG/DXF files.',
        'Improved efficiency by reducing manual drafting effort in Revit.',
      ],
      technologies: ['Revit API', 'C#', 'WPF'],
      category: 'BIM Automation',
    },
    {
      title: 'Facility Management Software – Maintenance Module',
      company: 'ITI, Civil Engineering Informatics Track (2022)',
      description:
        'Developed a facility management system module as a team project, focusing on maintenance workflows and data visualization.',
      fullDescription: [
        'Demonstrated team-based software engineering skills in a capstone project.',
        'Implemented maintenance workflows and data visualization features.',
        'Utilized ASP.NET, HTML/CSS, JavaScript, jQuery, jQuery UI, xBim, Chart.js, Datatable, and IFCTools.',
      ],
      technologies: ['ASP.NET', 'JavaScript', 'jQuery', 'xBim', 'Chart.js', 'Datatable'],
  ...
  const categories = ['GitHub Repositories'];

  const [activeCategory, setActiveCategory] = useState('GitHub Repositories');
    ...
    const fetchRepos = async () => {
      try {
        ...
        const res = await fetch(
          'https://api.github.com/users/ahmedfikry145975/repos?sort=updated&per_page=12'
        );
              ...
              <div key={i} className={styles.projectCard}>
                <div className={styles.projectContent}>
                  <div className={styles.projectHeader}>
                    <span className={styles.categoryBadge}>Loading…</span>
                  </div>
                  <h3 className={styles.projectTitle}>Loading repository…</h3>
                  <p className={styles.projectDescription}>Fetching your GitHub projects.</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className={styles.projectGrid}>
            {repos.map((repo) => (
              <div key={repo.id} className={styles.projectCard}>
                <div className={styles.projectContent}>
                  <div className={styles.projectHeader}>
                    <span className={styles.categoryBadge}>{repo.language || 'Repository'}</span>
